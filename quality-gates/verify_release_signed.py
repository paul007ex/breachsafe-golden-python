...
def verify_one(
    ...
    if _rc(["gh", "attestation", "verify", str(artifact), "--repo", repo,
            "--signer-workflow", signer_workflow]) != 0:
        return False, f"gh attestation verify failed: {artifact.name}"
    if _rc(["cosign", "verify-blob", "--bundle", str(bundle),
            "--certificate-oidc-issuer", OIDC_ISSUER,
            "--certificate-identity-regexp", signer_regexp, str(artifact)]) != 0:
...
def main(argv: list[str] | None = None) -> int:
    ...
    view = subprocess.run(
        ["gh", "release", "view", a.tag, "-R", a.repo, "--json", "assets", "--jq",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for d in dists:
            subprocess.run(
                ["gh", "release", "download", a.tag, "-R", a.repo, "-p", d,
                 "-p", f"{d}.sigstore", "-D", str(tmp)],
                capture_output=True, text=True, check=False)
            ok, msg = verify_one(tmp / d, tmp / f"{d}.sigstore", a.repo,
                                 a.signer_workflow, a.signer_identity_regexp)
